...
// === Add-New visibility fix: always mount photo buttons in edit mode ===
// If the DOM doesn't show any 'Photo:' label rows yet (new/empty record),
// fall back to a known list of photo headers so the upload buttons appear.
(function(){
  'use strict';
  if (window.__HFD_UPLOAD_INJECTOR_ADDNEW_FIX__) return;
  window.__HFD_UPLOAD_INJECTOR_ADDNEW_FIX__ = true;

  var modal   = document.getElementById('recordModal');
  var content = document.getElementById('modalContent');
  if (!modal || !content) return;

  // Known photo headers in the sheet (include exact trailing colons)
  var PHOTO_HEADERS = [
    'Photo:', 'Alarm Photo:', 'Roof Access Photo:', 'Elevator Shutoff Photo:',
    'Elevator Key Photo:', 'FDC Photo:', 'Sprinkler Riser Photo:', 'Panel Photo:'
  ];

  function sectionFor(header){
    var h = String(header||'').toLowerCase();
    if (/elev/.test(h)) return 'elevators';
    if (/fdc|alarm|sprinkler|riser|fire/.test(h)) return 'fire';
    ...
    if (/hazmat|chemical|flammable|tank/.test(h)) return 'hazmat';
    return 'other';
  }

  function ensureBar(secId){
    var secEl = document.getElementById('section-' + secId);
    if (!secEl) return null;
    var bar = secEl.querySelector('.per-section-uploads');
    if (bar) return bar;
    bar = document.createElement('div');
    bar.className = 'per-section-uploads';
    var h3 = secEl.querySelector('h3');
    if (h3 && h3.nextSibling) secEl.insertBefore(bar, h3.nextSibling); else secEl.prepend(bar);
    return bar;
  }

  function currentPhotoHeadersFromDOM(){
    var ks = Array.from(content.querySelectorAll('.kv .k')).map(function(el){ return (el.textContent||'').trim(); });
    return ks.filter(function(k){ return /photo/i.test(k); });
  }

  function mountUploads(){
    if (!modal.classList.contains('editing')) return;

    var headers = currentPhotoHeadersFromDOM();
    if (!headers.length){
      // DOM has no photo rows yet (new record): mount from known headers
      headers = PHOTO_HEADERS.slice();
    }

    headers.forEach(function(header){
      var sec = sectionFor(header);
      var bar = ensureBar(sec);
      if (!bar) return;
      if (bar.querySelector('[data-hfd-upload="'+header+'"]')) return;
      ...
      input.addEventListener('change', function(){ 
        // Let the existing injector's change handler (if any) process it;
        // otherwise dispatch a custom event other code can hook.
        if (!input.files || !input.files.length) return;
        var ev = new CustomEvent('hfd:upload-files', { detail: { header: header, input: input } });
        input.dispatchEvent(ev);
      });
    });
  }

  // Observe modal state and try a few times while editing
  try{
    var mo = new MutationObserver(function(){
      if (modal.open && modal.classList.contains('editing')){
        mountUploads();
        setTimeout(mountUploads, 0);
        setTimeout(mountUploads, 200);
        setTimeout(mountUploads, 600);
      }
    });
  ...
  // Initial pass
  setTimeout(function(){
    if (modal.open && modal.classList.contains('editing')) mountUploads();
  }, 100);
})();
